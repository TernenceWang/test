...
static const int __sizeintmut4__=4*sizeof(int);
BOOL YUVToRGB(LONG biBitCount,LONG VideoWidth,LONG VideoHeight,unsigned char* pYUVData,unsigned char* pRGBData,int lPitch)
{
	BOOL bSucc= FALSE;	
	LONG RGBLineSize=lPitch;//VideoWidth * biBitCount/8;
	LONG YLineSize=*(int *)pYUVData; 
	LONG ULineSize=*(((int *)pYUVData)+1); 
	LONG VLineSize=*(((int *)pYUVData)+2);
	unsigned char *YDataPos=(unsigned char *)(pYUVData + __sizeintmut4__);
	unsigned char *CbDataPos=(unsigned char *)(pYUVData + __sizeintmut4__ + VideoHeight * YLineSize);
	unsigned char *CrDataPos=(unsigned char *)(pYUVData + __sizeintmut4__ + VideoHeight * YLineSize + (VideoHeight* ULineSize)/2);

	if (biBitCount == 32)
	{
		yv12_to_bgra_c(pRGBData,RGBLineSize,YDataPos/*y*/,CrDataPos/*V*/,CbDataPos/*u*/,
			YLineSize,ULineSize,VideoWidth,VideoHeight,0);
		bSucc= TRUE;
	}else if(biBitCount == 24)
	{
		yv12_to_rgb_c(pRGBData,RGBLineSize,YDataPos/*y*/,CrDataPos/*V*/,CbDataPos/*u*/,
			YLineSize,ULineSize,VideoWidth,VideoHeight,0);
		bSucc= TRUE;
	}
	else if (biBitCount == 16)
	{
		yv12_to_rgb565_c(pRGBData,RGBLineSize,YDataPos/*y*/,CrDataPos/*V*/,CbDataPos/*u*/,
			YLineSize,ULineSize,VideoWidth,VideoHeight,0);
		bSucc= TRUE;
	}
	else if (biBitCount == 8)//²»Ö§³Ö
	{
	}
	return bSucc;
}
